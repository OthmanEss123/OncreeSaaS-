...
export default function EditConsultantPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Parse skills
        let skillsString = ''
        if (consultantData.skills) {
          try {
            const parsed = JSON.parse(consultantData.skills)
            skillsString = Array.isArray(parsed) ? parsed.join(', ') : consultantData.skills
          } catch {
            skillsString = consultantData.skills
          }
        }
        ...
        setFormData({
          ...
          skills: skillsString,
          daily_rate: consultantData.daily_rate || 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(`/rh/consultant/${consultant.id}`)
      }, 2000)
