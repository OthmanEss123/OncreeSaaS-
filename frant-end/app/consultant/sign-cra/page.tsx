...
export default function SignCRAPage() {
  ...
  const handleSign = async () => {
    if (!signatureData) {
      setError('Veuillez signer dans la zone prévue à cet effet')
      return
    }

    if (!confirm(`Êtes-vous sûr de vouloir signer le CRA de ${workLog?.monthName} ?\n\nCette action est irréversible et certifie que les informations du CRA sont exactes.`)) {
      return
    }
    ...
    try {
      ...
      // Envoyer la signature avec les données du CRA
      const result = await WorkScheduleAPI.signCRA(month, year, signatureData)
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <motion.button
            ...
            whileHover={!signing ? { scale: 1.05 } : {}}
            whileTap={!signing ? { scale: 0.95 } : {}}
          >
            {signing ? (
              <>
